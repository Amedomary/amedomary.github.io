...
define(["jquery"], function($) {
  ...
  const textBlockPosition =
    $textBlock.getBoundingClientRect().top + pageYOffset;
  ...
  const $toTopScrollBlock = document.getElementsByClassName(
    "to-top-scroller"
  )[0];
  ...
  function positionLine(e) {
    $hr.css({ transform: `translateY(${e.y}px)` });
  }
  ...
  $pageWithScroll.scroll(function() {
    ...
    if (scrollPositionTop > textBlockPosition) {
      ...
      $headerBg.css({ transform: `translateY(-${scrollPositionTop / 3}px)` });
    }
  });
});
